...
//       if (serviceFactory === expect.anything(paymentServiceFactory)) {
//         return mockPaymentService;
//       } else if (serviceFactory === expect.anything(orderServiceFactory)) {
...
//   test("Submits the form with valid values; Expect update, paymentService.create and orderService.create to be called", async () => {
//     const mockUserInformation = {
//       token: mockToken,
//       userId: mockUserId,
//     };
...
//     await waitFor(() => {
//       // Ensure userCardDetailsService.update is called
//       expect(mockUserCardDetailsService.update).toHaveBeenCalledWith(
//         userId,
//         submitData
//       );

//       // Ensure paymentService.create is called
//       expect(mockPaymentService.create).toHaveBeenCalledWith(userId, submitData);

//       // Ensure orderService.create is called
//       expect(mockOrderService.create).toHaveBeenCalledWith(userId);
//     });
...
//   test("Submits the form with invalid; Expect update, paymentService.create and orderService.create not to be called; Expect errors", async () => {
//     const mockUserInformation = {
//       token: mockToken,
//       userId: mockUserId,
//     };
...
describe("CardDetailsForm Component", () => {
  ...
  test("Submits the form with valid values and popupCloseHandler; Expect update and popupCloseHandler to be called, paymentService.create and orderService.create not to be called", async () => {
    ...
    await waitFor(() => {
      ...
      // Ensure popupCloseHandler is called
      expect(mockPopupCloseHandler).toHaveBeenCalled();

      // Ensure paymentService.create and orderService.create are not called
      expect(mockPaymentService.create).not.toHaveBeenCalled();
      expect(mockOrderService.create).not.toHaveBeenCalled();
    });
  ...
  test("Submits the form with valid values without popupCloseHandler; Expect update, paymentService.create and orderService.create to be called", async () => {
    ...
    await waitFor(() => {
      ...
      // Ensure paymentService.create and orderService.create are called
      expect(mockPaymentService.create).toHaveBeenCalledWith(userId, submitData);
      expect(mockOrderService.create).toHaveBeenCalledWith(userId);

      // Ensure popupCloseHandler is not called
  ...
  // Add additional tests for invalid, expired, and empty values with both cases if needed
});
