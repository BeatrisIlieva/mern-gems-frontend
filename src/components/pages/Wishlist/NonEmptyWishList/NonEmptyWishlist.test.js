...
// Mock the useWishlistContext hook
...
describe("NonEmptyWishlist Component", () => {
  test("renders InfoMessage and Content components when wishlist has items", () => {
    // Mock data for wishlist context
    const mockWishlistItems = [
      { _id: "1", category: { title: "Jewelry" }, color: { title: "Red" } },
      { _id: "2", category: { title: "Accessories" }, color: { title: "Blue" } },
    ];
    ...
    // Check InfoMessage rendering
    expect(screen.getByText(`Your Wish List (${mockWishlistTotalQuantity})`)).toBeInTheDocument();
    expect(screen.getByText("Your favorite item(s) are below. Wishes can come true, especially when you dream.")).toBeInTheDocument();

    // Check Content components rendering
  ...
  test("does not render InfoMessage or Content components when wishlist is empty", () => {
    ...
    // Check that InfoMessage and Content components are not rendered
    expect(screen.queryByText("Your Wish List")).not.toBeInTheDocument();
    expect(screen.queryByText("This list is empty. Explore and add something you love.")).not.toBeInTheDocument();
